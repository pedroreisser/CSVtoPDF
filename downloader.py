...
CONFIG_PATH = Path.home() / ".pdf_harvester_config.json"
...
def read_file(file_path):
    """Retorna (header, data_rows, delimiter, doi_col_idx, title_col_idx).

    doi_col_idx / title_col_idx ficam None quando não é possível detectar
    ...
    return header, data_rows, delimiter, doi_col, title_col
...
def extract_articles(data_rows, doi_col, title_col):
    articles = []
    for row in data_rows:
        ...
        if not doi and not title:
            continue
        articles.append({"doi": _clean_doi(doi), "title": title})
    return articles
...
def query_unpaywall(doi, email, session):
    """Retorna (pdf_url, erro). Se erro is None, pdf_url é válido."""
    ...
    if not best or not best.get("url_for_pdf"):
        return None, "sem versão OA"
    return best["url_for_pdf"], None
...
def process_articles(articles, email, dest_folder, msg_queue, cancel_event,
    ...
    for i, article in enumerate(articles, start=1):
        ...
        status, detail, filename = _process_one(doi, title, email, dest_folder, session)

        result = {"doi": doi, "title": title, "status": status, "detail": detail, "filename": filename}
...
def _process_one(doi, title, email, dest_folder, session):
    ...
    try:
        pdf_url, err = query_unpaywall(doi, email, session)
        if err:
            return ("no_oa" if err == "sem versão OA" else "error"), err, ""

        dest_path = _unique_path(dest_folder / (sanitize_filename(title, doi) + ".pdf"))
        try:
            download_pdf(pdf_url, dest_path, session)
            return "downloaded", "ok", dest_path.name
        except Exception:
...
def build_not_found_html(rows):
    ...
    rows: lista de dicts com 'doi', 'title' e (opcional) 'detail'.
    ...
    cab_motivo = "<th>Motivo</th>" if tem_motivo else ""
    return _HTML_TEMPLATE.format(
        total=len(rows),
        cab_motivo=cab_motivo,
        linhas="\n".join(linhas),
    )
...
navegador. Você também pode marcar/desmarcar manualmente. <span id="contador"></span></p>
<input id="busca" type="search" placeholder="Filtrar por título ou DOI…"
       oninput="filtrar(this.value)" autofocus>
<table>
<thead><tr><th>✓</th><th>#</th><th>Título</th><th>DOI</th>{cab_motivo}</tr></thead>
<tbody id="corpo">
{linhas}
</tbody>
</table>
...
const STORE = 'pdfharvester_abertos';
...
def demo():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        header, rows, delim, doi_col, title_col = read_file(csv_path)
        assert delim == ","
        assert doi_col == 0 and title_col == 1
        articles = extract_articles(rows, doi_col, title_col)
        assert articles == [
            {"doi": "10.1/abc", "title": "Um Titulo"},
            {"doi": "", "title": "Sem doi"},
        ]

        wos_path = tmp / "wos_raw.txt"
        wos_path.write_text("PY\tDI\tTI\n2020\thttps://doi.org/10.2/xyz\tOutro Titulo\n", encoding="utf-8")
        header, rows, delim, doi_col, title_col = read_file(wos_path)
        assert delim == "\t"
        assert doi_col == 1 and title_col == 2
        articles = extract_articles(rows, doi_col, title_col)
        assert articles == [{"doi": "10.2/xyz", "title": "Outro Titulo"}]
        ...
        resultados = [
            {"doi": "10.1/a", "title": "Baixado", "status": "downloaded", "detail": "ok", "filename": "x.pdf"},
            {"doi": "10.2/b", "title": "Sem OA", "status": "no_oa", "detail": "sem versão OA", "filename": ""},
            {"doi": "", "title": "Sem DOI <script>", "status": "error", "detail": "sem DOI", "filename": ""},
        ]
